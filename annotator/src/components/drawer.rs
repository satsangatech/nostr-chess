...
pub fn left_drawer(props: &LeftDrawerProps) -> Html {
    ...
    let drawer_class = if *is_open {
        "fixed left-0 top-0 z-50 h-full w-fit max-w-1/2 transform bg-white shadow-lg transition-transform duration-300 ease-in-out translate-x-0"
    } else {
        "fixed left-0 top-0 z-50 h-full w-fit max-w-1/2 transform bg-white shadow-lg transition-transform duration-300 ease-in-out -translate-x-full"
    };

    let overlay_class = if *is_open {
        "fixed inset-0 z-40 bg-black/50 px-8 transition-opacity duration-300 opacity-100"
    } else {
        "fixed inset-0 z-40 bg-black/50 px-8 transition-opacity duration-300 pointer-events-none opacity-0"
    };
