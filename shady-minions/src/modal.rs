...
pub fn left_modal(props: &ModalProps) -> Html {
    ...
    let modal_class = if *is_open {
        "fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center z-70 transition-transform duration-300 ease-in-out scale-100 pointer-events-none"
    } else {
        "fixed top-0 left-0  right-0 bottom-0 flex justify-center items-center z-70 transform transition-transform duration-300 ease-in-out scale-0 pointer-events-none"
    };

    let overlay_class = if *is_open {
        "fixed top-0  right-0 bottom-0 left-0  z-60 bg-black/50 transition-opacity duration-300 opacity-100"
    } else {
        "fixed top-0  right-0 bottom-0 left-0  z-60 bg-black/50 transition-opacity duration-300 pointer-events-none opacity-0"
    };
